...
```{r setup}
#| echo: false
pacman::p_load(tidyverse, ggthemes, plotly, broom, broom.helpers)
...
令和8年度(2026年度)の松浦プレゼミでは、**データに強い経営学部生**となるため、データ分析のツールとしてもはやデファクトスタンダードとなっている(いいすぎ)プログラミング言語**R**を使って、様々な分析を行うためのスキルの修得を目指します。
とりわけ今年度は、非常に優れたテキストである田頭拓己 (2025)「マーケティング・データ分析」有斐閣が出版されたこともあり、プレゼミの教科書として本書を用います。
ビジネスのデータと適切な統計スキルを組み合わせることで、データ分析に強い経営学部生になることを目指します。
...
立命館大学では、株価データや会計データ、POSデータといったデータベースを契約しているので、企業分析を行うための豊富なデータを利用することができます。
さらに自分で集めたアンケートのデータや、Webスクレイピングで集めたデータなど、公開されているオープンデータといった大規模データを分析できるスキルは、会計・ファイナンスだけでなく、組織、戦略、マーケティングと様々な分野で役に立つでしょう。

またRで分析した結果をレポートにまとめる際に有用なツールとして、**Quarto**を活用します。
**Quarto**(クアルト)とは、Markdown記法で書かれた文章の中に、RだけでなくPythonなど他の言語も組み込める便利なツールです。
Markdown記法やYAML記法を少し覚える必要がありますが、Quartoで書かれた文章から、PDFやHTML、MS Wordなど様々な形式のレポートを作成することができます。

今までのように、MS Excelで作図・作表を行い、それをMS WordやMS Powerpointに貼り付けるという作業をやめて、
Visual Studio CodeやRstudioといったIDE環境でQuartoを用いて、1カ所でデータ分析とレポート作成を行いましょう。
...
:::{.callout-note}
## デメリット
- コンピューターの知識が必要(ディレクトリとかパスとか)
- 導入や設定が手間
- 命令文でデータ操作するため、ぱっと見わかりにくい
- R言語の文法と単語を覚える必要がある
...
マイクロソフト社の表計算ソフト「Excel」は非常に便利だけれど，データ分析のツールとしては以下のような特徴があります。
